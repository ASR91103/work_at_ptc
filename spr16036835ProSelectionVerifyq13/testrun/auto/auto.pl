...
if ($dcad_server eq "") {
  ...
  `cp $trail_path/test/$ENV{R_AUTOTXT} "$rundir"` if (! -e $autotxt);
...
sub get_secret {
   ...
   if (lc($ENV{CREO_SAAS_CLUSTER}) eq 'production' || lc($ENV{CREO_SAAS_CLUSTER}) eq 'prod') {
     ...
     $curl_data = `$csh_cmd "curl -s --ssl-no-revoke --cacert $ENV{PTC_TOOLS}/cacerts/ptcrootca_ptcissuingca.pem -X POST https://token.ptcnet.ptc.com/secret/get_atlas_collab_creds" `;   
   }
